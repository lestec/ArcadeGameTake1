...
var Enemy = function(x,y,speed) {
    ...
    this.x = x;
    //this.y = 58;//center at 0 its in the water
    this.y = y + 58;
    //3rd parameter to enemy class(Inside constructor,
    //setting this.speed to value of parameter.)
    this.speed = speed;
...
Enemy.prototype.update = function(dt) {
    ...
    //405 stops at edge, but not smooth transition
    if(this.x < 502) {
        //move 
...
//startY places player 5 blocks down from top row/top left 0,0
    //may need to use subt to get centered better when move?    
...
//set bk to bottom middle with above calc -- use for reset
        this.x = this.startX;
        this.y = this.startY;
//setting winGame to try to get game to stop moving and modal to show
        this.winGame = false;
...
//HAVE TO USE update here for engine.js
    update() {
    //check collision here
        this.checkCollision();
        if (this.y <= 0) {
            console.log("Win");
    //setting to try to stop animation after win
            this.winGame = true;        
            //stopGame();
            gmModal();
        }
...
*/
//trying to figure out where to add gameOver(stopGame)
    /*not working cannot define win, go to engine??
    stopGame() {
...
/*see below in keypress fx hanleInput
...
//const bug4 = new Enemy(-101, 125, 100);
const allEnemies = [];
//to push bugs into array -- took out bug for now to test
allEnemies.push(bug1,bug2,bug3);
console.log(allEnemies);

/* Having prob with game stopping so trying to move this into
player constructor***

//adding gameOver function to stop game
function gameOver() {
    //allEnemies = [];
...
*/
//gm over modal from sweetalerts 
function gmModal() {
    swal({
        title: 'Game Over',
        text: 'You Win! ',
